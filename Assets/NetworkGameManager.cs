...
public class NetworkGameManager : MonoBehaviourPunCallbacks // Photon Realtime 用のクラスを継承する
{
    ...
    [SerializeField] Transform m_extraSpawnPoint;
    ...
    private void SpawnPlayer()
    {
        ...
        Transform spawnPoint = m_extraSpawnPoint;
        ...
        }   // 誰かが抜けて後から入ってきた時は m_extraSpawnPoint に spawn させる
        ...
        // 自分だけ入力を有効にする
        player.GetComponent<TankController>().enabled = true;

        if (actorNumber < m_spawnPoints.Length + 1)
        {
            // 最初の m_spawnPoints.Length 人は中央を向く
            player.transform.LookAt(Vector3.zero + Vector3.up * 0.5f);
        }
        else
        {
            // m_extraSpawnPoint に spawn させる時はランダムな方向を向く
            player.transform.Rotate(player.transform.up, Random.Range(0, 360f));
        }
    }
